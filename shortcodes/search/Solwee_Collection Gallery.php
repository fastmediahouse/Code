add_shortcode('solwee_collection_gallery', function () {
    ...
    if (!$product || empty($product->archiveID)) return '';

    $collection = $product->archiveID;

    $search_response = wp_remote_post("https://api.solwee.com/api/v2/search/images/editorial", [
        'headers' => ['Content-Type' => 'application/json', 'Authorization' => "Bearer {$token}", 'X-WebID' => $webID],
        'body' => json_encode([
            'archiveID' => $collection,
            'limit' => $limit,
            'offset' => 0
        ])
    ]);
    ...
    <div style="margin-top:40px;">
        <h3>More images from this collection</h3>
        ...
        ?>
            <div class="solwee-tile" style="break-inside:avoid;margin-bottom:20px;position:relative;box-shadow:0 2px 6px rgba(0,0,0,0.1);border-radius:6px;overflow:hidden;">
                <a href="/image-detail/?productID=<?= $id ?>" target="_blank">
                    <img src="<?= $thumb ?>" onerror="this.onerror=null;this.src='<?= $fallback ?>';" alt="Collection Image" style="width:100%;display:block;" loading="lazy" />
                </a>
        ...
        <div style="margin-top:20px;text-align:center;">
            <a href="/search-results/?q=<?= urlencode($collection) ?>&category=editorial" 
