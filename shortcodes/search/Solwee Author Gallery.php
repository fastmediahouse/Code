add_shortcode('solwee_author_gallery', function () {
    ...
    if (!$product || empty($product->author)) return '';
    ...
    <div style="margin-top:40px;">
        <h3>More images from <?= esc_html($author) ?></h3>
        ...
        ?>
            <div class="solwee-tile" style="break-inside:avoid;margin-bottom:20px;position:relative;box-shadow:0 2px 6px rgba(0,0,0,0.1);border-radius:6px;overflow:hidden;">
                <a href="/image-detail/?productID=<?= $id ?>" target="_blank">
                    <img src="<?= $thumb ?>" onerror="this.onerror=null;this.src='<?= $fallback ?>';" alt="Author Image" style="width:100%;display:block;" loading="lazy" />
                </a>
